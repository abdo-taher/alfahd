...
export function MobileCTABar({
  locale,
  whatsappNumber = "966114459222",
  phoneNumber = "+966114459222",
}: MobileCTABarProps) {
  const pathname = usePathname();
  const isAr = locale === "ar";

  // Hide on portal and request-quote pages
  if (pathname.includes("portal") || pathname.includes("request-quote")) return null;

  return (
    <div
      className="fixed bottom-0 left-0 right-0 z-50 md:hidden flex border-t border-gray-200 bg-white shadow-lg"
      role="navigation"
      aria-label={isAr ? "اتصل بنا" : "Contact us"}
    >
      <a
