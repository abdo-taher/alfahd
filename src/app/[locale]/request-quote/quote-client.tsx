...
export function QuoteClient() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Persist to localStorage so the portal dashboard can track this RFQ
    try {
      ...
      const newEntry = {
        ...
        fullName: form.name,
        companyName: form.company || form.name,
        email: form.email,
        phone: form.phone,
        projectType: services[form.service as ServiceKey] ?? form.service,
        scope: [form.projectLocation, form.projectSize, form.projectDescription]
          .filter(Boolean)
          .join(" | "),
          ...
          <div className="max-w-3xl mx-auto">
            {formState === "success" ? (
              <div className="bg-surface-container-low border border-outline-variant rounded-lg p-16 text-center">
                ...
                <p className="text-body-lg text-on-surface-variant mb-8">{ft.success}</p>
                <a
                  href={`/${locale}/portal`}
                  className="inline-flex items-center gap-2 bg-primary text-on-primary px-6 py-3 rounded text-label-bold hover:bg-primary-container transition-colors"
                >
                  {locale === "ar" ? "تابع طلبك في بوابة العملاء" : "Track your request in the Portal"}
                </a>
              </div>
            ) : (
